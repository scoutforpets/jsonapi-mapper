...
function BookshelfAdapter(data, type, baseUrl, serializerOptions, options) {
  ...
  if (!_isDataEmpty(data)) {
    ...
    template.dataLinks = _buildSelfLink(baseUrl, type, model.get('id'));
...
function _buildTopLevelLinks(baseUrl, type, queryParams) {
  return _buildSelfLink(baseUrl, type, null, queryParams);
}
 ...
 * @return {[type]}         [description]
 */
function _buildSelfLink(baseUrl, modelType, id, queryParams) {
  return {
