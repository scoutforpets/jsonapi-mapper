...
function BookshelfAdapter(data, type, baseUrl, serializerOptions, options) {
  ...
  // If data is a collection, but only contains a single model, then process that
  // data as a singular model and do not return a JSON API array
  if (isCollection && data.models.length == 1) {
    isCollection = false;
    data = data.models[0];
  }
...
function _buildPaginationLinks(baseUrl, type, pagination, queryParams) {
  ...
  return {
    ...
    first: function(dataSet, model) {
      ...
      link += '?page[limit]=' + pagination.limit + '&page[offset]=0';
