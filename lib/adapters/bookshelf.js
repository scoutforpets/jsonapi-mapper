...
function BookshelfAdapter(data, type, baseUrl, serializerOptions, includeRelations) {
  ...
  if (!_isDataEmpty(data)) {
    ...
    // Include relations in the response unless specified in the
    // serializerOptions or passed directly to the adapter. Default is true.
    var included = true;

    if (includeRelations !== undefined) {
      included = includeRelations;
    } else if (serializerOptions.includeRelations !== undefined) {
